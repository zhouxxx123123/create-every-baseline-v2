...
If the bounded target persists operational business state, also locate and verify its `READY_FOR_SPEC` operational data-design receipt with `data-design/scripts/verify_data_design_receipt.py`. Require the exact design ID, target, boundary, stable contract IDs, authority identities, logical model, and downstream handoff. Stop and return to `data-design` when the receipt is absent, stale, mismatched, or fails verification. Do not derive tables from prototype fixtures or let the specification change object identity, invariants, lifecycle, permissions, transaction boundaries, concurrency, idempotency, or recovery contracts. A target with no persisted business state records that the data-design stage is not applicable.
...
3. Write the spec using the template below, then publish it to the project issue tracker. Apply the `ready-for-agent` triage label - no need for additional triage. Record the exact Product Readiness receipt and, when applicable, operational data-design receipt in the specification. Reference stable data-design IDs instead of copying or weakening their contracts. If the tracker supports a timeline note, append a separate `Consumed by <spec>` note without editing either immutable receipt.
