...
The root object uses `schema_version: "1.2"` and contains:
...
`package_acceptance` stays `PENDING` while the design is being derived. Set it to `ACCEPTED` only after the user accepts the bounded package. Record the accepter display label, stable `accepted_by_ref`, timezone-aware time, a locally verifiable confirmation reference and digest, and every accepted `PROPOSED_ARCHITECTURE` ID. This is package-level acceptance, not a product decision and not permission to confirm blocked questions.

`source_authorities` entries require `stable_id`, `ref`, `authority_kind`, `authority_status`, `version`, `content_sha256`, `currentness_status`, and a non-future timezone-aware `currentness_checked_at` for confirmed authority. Only current canonical product authority may use `CONFIRMED` or `CONFIRMED_AND_VALIDATED`. Research, implementation, prototypes, and spikes keep their actual evidence role. Readiness validation resolves local files, verifies the declared digest, and verifies a Markdown heading fragment when one is present. An opaque URI or nonexistent anchor cannot establish machine-verified authority.

`prototype_evidence` entries require `stable_id`, `ref`, `source_refs`, `admitted_ids`, `review_status`, `version`, and exact locally verifiable manifest, artifact, and fixture references plus SHA-256 digests. Only `review_status: ADMITTED` may name `admitted_ids`, and a material item may cite that prototype only when its own ID appears in the admitted set.

`admission` identifies the exact passing Product Readiness receipt. For the bundled verifier, both `version` and `verifier` are `product-readiness-receipt/v1`; the validator invokes that verifier and requires a matching target. A project-declared equivalent must register a verifier extension first. A path, digest, verdict string, or arbitrary executable name cannot self-declare admission.

`quality_review.findings` stores review findings with `finding_id`, `severity`, `status`, `affects_gate`, and `message`. A passing review records a stable reviewer identity plus locally verifiable review evidence and digest. A readiness gate requires `quality_review.status: PASSED` and rejects an open P1/P2 finding affecting that gate. Review must follow the latest authority currentness check; acceptance must follow review.
...
| `transaction_boundaries` | command refs, writes, external effects, closed atomicity mode, atomicity detail, closed partial-success mode, partial-success policy |
...
| `unknown_outcome_contracts` | command refs, unknown state, authority, verification, closed retry mode, retry detail, resolution fact |
...
Use these closed values where the contract requires an enum:

- object `content_mode`: `STORED`, `REFERENCED`, or `HYBRID`;
- relationship `cardinality`: `ONE_TO_ONE`, `ONE_TO_MANY`, `MANY_TO_ONE`, or `MANY_TO_MANY`;
- consistency `consistency_model`: `IMMEDIATE`, `OPTIMISTIC_CAS`, `SERIALIZABLE`, `EVENTUAL`, or `EXTERNAL_AUTHORITY`;
- transaction `atomicity_mode`: `ATOMIC`, `SAGA`, or `EXTERNAL_NON_ATOMIC`;
- transaction `partial_success_mode`: `NONE`, `RECORDED_AND_RECOVERABLE`, or `COMPENSATED`;
- unknown-result `retry_mode`: `VERIFY_THEN_RETRY`, `NO_AUTOMATIC_RETRY`, or `MANUAL_RECOVERY`;
- physical `adapter_kind`: `POSTGRESQL` or `SUPABASE`;
- blocker `blocks`: `LOGICAL`, `PHYSICAL`, or `BOTH`.

Every active permission contract for a material write sets `execution_time_revalidation: true`. A design with multiple active objects explicitly models their relationships.
...
A ready root `status`, `downstream_handoff.requested_gate`, and consumer must agree exactly. `READY_FOR_SPEC` routes to `to-spec`; `READY_FOR_TICKETS` routes to `to-tickets`. A stronger-looking status cannot bypass its own physical checks by requesting a weaker gate.
...
Each physical or end-to-end test used for `READY_FOR_TICKETS` records `evidence_ref`, `evidence_sha256`, `runner`, non-future `run_at`, and `result: PASS`. Evidence bytes are verified when a design path is available. Each coverage category must cover a matching contract type; category labels alone do not prove safety.
...
The Markdown and JSON must describe the same design. The Markdown must contain the design ID, target, and every active machine-contract stable ID, and must not contain unresolved placeholder tokens. The V2 receipt hashes and verifies both files, binds its timestamp and complete machine identity into the receipt ID, and regenerates the visible summary during verification. Semantic parity remains part of quality review, so do not put untracked material decisions only in prose.
